...
function Footer() {
  return (
    ...
    data-scroll
    data-scroll-speed="-.1"
    className='w-full h-screen bg-[#F1F1F1] pt-24 pb-8 px-14 rounded-tr-3xl rounded-tl-3xl flex'>
        <div className='w-1/2 h-full flex flex-col justify-between text-[#212121] '>
            <h1 className='text-[7vw] leading-none font-["Oswald"] font-bold tracking-tight '>EYE- <br /> OPENING</h1>
        ...
        <div className='w-1/2 h-full'>
            <h1 className='text-[#212121] text-[7vw] leading-none font-["Oswald"] font-bold tracking-tight'>PRESENTATIONS</h1>
            <div className='flex'>

                <div className='w-1/2 h-full flex flex-col'>
                    <div className='links flex flex-col text-black mt-8'><span className='mb-4'>S:</span>
                        {["Instagram", "Behance", "Facebook", "Linkedin"].map((item, index) => (
                        <a key={index} className={`capitalize underline cursor-pointer`}>{item}</a>
                        ))}
                    </div>
                    <div className='links flex flex-col text-black mt-8'>
                        <span className='mb-4'>L:</span>
                        {["202-1965 W 4th Ave", "Vancouver, Canada", "30 Chukarina St", "Lviv, Ukraine"].map((item, index) => (
                            <a key={index} className={`capitalize underline cursor-pointer ${index === 2 ? 'mt-6' : ''}`}>
                                {item}
                            </a>
                        ))}
                    </div>
                    ...
                    <div className='text-[#F5A8A8] mt-10 pb-8 '>© ochi design 2024. Legal Terms</div>
                </div>

                <div className='w-1/2 h-[53vh] mt-40 flex flex-col items-end justify-center px-4 '>
                    <div>
                        <div className='links flex flex-col text-black mt-8'><span className='mb-4'>M:</span>
                            {["Home", "Services", "Our work", "About us", "Insights", "Contact us"].map((item, index) => (
                            <a key={index} className={`capitalize underline cursor-pointer`}>{item}</a>
                            ))}
                        </div>
                        <div className='text-[#F5A8A8] mt-24 '>Website by Obys</div>
                    </div>
                </div>
            </div>
        </div>
